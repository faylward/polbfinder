...
score_dict = {'tecti':float(10), 'arc1':float(10), 'arc2':float(10), 'arc3':float(10), 'arcmis':float(10), 'adeno':float(10), 'baculo':float(10), 'ea':float(10), 'ed':float(10), 'ee':float(10), 'ez':float(10), 'gv':float(10), 'herpes':float(10), 'mv':float(10), 'plv':float(10)}
...
def hmm_launcher(folder, redo):
	...
	for files in os.listdir(folder):
		if files.endswith(".faa"):
			...
			cmd = "hmmsearch --cpu 4 -E 1e-1 --domtblout "+ speci_dom_output +" "+ speci_db + " " + input_file
			#print(cmd)
		...
		if filenames.endswith(suffix):
			...
			for line in f.readlines():
				if line.startswith("#"):
					...
					if score > bit_dict[ids] and domain_evalue < 1e-1:
						ids_hit = ids +"."+ hit
						start = int(list1[15])
						end   = int(list1[16])
...
def run_program(inputdir, project, prox, cpus, redo, allhits, markerset, concat):
	...
	if markerset == "rnap":
		...
		cog_set = ['tecti', 'arc1', 'arc2', 'arc3', 'arcmis', 'adeno', 'baculo', 'ea', 'ed', 'ee', 'ez', 'gv', 'herpes', 'mv', 'plv']
	elif markerset == "ribo":
			...
			for cog in cog_set:
				...
				for main_hit in main_hits:
					...
					if main_hit in protein_tally:
						...
						pass
					elif cog in ['tecti', 'arc1', 'arc2', 'arc3', 'arcmis', 'adeno', 'baculo', 'ea', 'ed', 'ee', 'ez', 'gv', 'herpes', 'mv', 'plv']:
...
def main(argv=None):
	...
	args_parser.add_argument('-m', '--markerset', required=False, default=str('polb'), help='HMM database to use. Options are "all", "ribo", "rnap" or "ribo_rnap". See README for details')
